...
import { z } from "zod";
import { Team, formatTeam, formatTeamsList } from "./tools/team";
import { makeStatboticsRequest } from "./utils/web-requests";
import { formatYearStats, formatYearsList, YearData } from "./tools/year";
import { TeamYear, formatTeamYear, formatTeamYearsList } from "./tools/team-year";
import { Event, formatEvent, formatEventsList } from "./tools/event";
import { TeamEvent, formatTeamEvent, formatTeamEventsList } from "./tools/team-event";
import { Match, formatMatch, formatMatchesList } from "./tools/match";
import { TeamMatch, formatTeamMatch, formatTeamMatchesList } from "./tools/team-match";
...
export class MyMCP extends McpAgent {
	...
	async init() {
		...
		this.server.tool(
			"get-team-years",
			"Query multiple years of a team's data with optional filters",	
			{
		...
		this.server.tool(
			"get-matches",
			"Query multiple matches a team has participated in with optional filters",
			{
				team: z.number().int().min(0).optional().describe("Team number (no prefix), e.g. 5511"),
				year: z.number().int().min(2002).max(2025).optional().describe("Four-digit year"),
				event: z.string().optional().describe("Event key, e.g. 2019ncwak"),

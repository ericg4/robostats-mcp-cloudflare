...
export function formatTeamMatch(teamMatch: TeamMatch): string {
  ...
  const otherKeys = Object.keys(breakdown).filter(key => 
    !['total_points', 'auto_points', 'teleop_points', 'endgame_points'].includes(key)
  );
  ...
  const autoPercent = breakdown.total_points > 0 ? (breakdown.auto_points / breakdown.total_points * 100) : 0;
  const teleopPercent = breakdown.total_points > 0 ? (breakdown.teleop_points / breakdown.total_points * 100) : 0;
  const endgamePercent = breakdown.total_points > 0 ? (breakdown.endgame_points / breakdown.total_points * 100) : 0;
  ...
  lines.push(`💡 This shows Team ${teamMatch.team}'s individual contribution to their ${teamMatch.alliance} alliance`);
...
export function formatTeamMatchesList(teamMatches: TeamMatch[]): string {
  ...
  teamMatches.forEach((teamMatch, index) => {
    ...
    lines.push(`   🏁 ${teamMatch.alliance.toUpperCase()} Alliance | ${teamMatch.elim ? "Elimination" : "Qualification"} | Status: ${teamMatch.status}`);
    ...
    // Performance summary
    const breakdown = teamMatch.epa.breakdown;
    lines.push(`   📊 EPA: ${teamMatch.epa.total_points.toFixed(1)} | Post-Match: ${teamMatch.epa.post.toFixed(1)}`);
    lines.push(`   🎮 Contribution: Auto ${breakdown.auto_points.toFixed(1)} | Teleop ${breakdown.teleop_points.toFixed(1)} | Endgame ${breakdown.endgame_points.toFixed(1)}`);

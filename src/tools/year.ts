...
export function formatYearStats(yearData: any): string {
	...
	if (yearData.score_mean !== undefined) {
		...
		if (yearData.breakdown?.no_foul_mean)
			lines.push(`Score (no fouls): ${yearData.breakdown.no_foul_mean}`);
		if (yearData.breakdown?.foul_mean)
			lines.push(`Average Fouls: ${yearData.breakdown.foul_mean} points`);
		lines.push("");
	}
	...
	if (yearData.breakdown) {
		...
		if (yearData.breakdown.auto_points_mean)
			lines.push(`Autonomous: ${yearData.breakdown.auto_points_mean} points`);
		if (yearData.breakdown.teleop_points_mean)
			lines.push(`Teleoperated: ${yearData.breakdown.teleop_points_mean} points`);
		if (yearData.breakdown.endgame_points_mean)
			lines.push(`Endgame: ${yearData.breakdown.endgame_points_mean} points`);
		lines.push("");
		...
		const gameElements = Object.entries(yearData.breakdown)
			.filter(
				([key]) =>
					!key.includes("points_mean") &&
					!key.includes("rp_mean") &&
					!key.includes("total_") &&
					!key.includes("foul") &&
					!key.includes("tiebreaker"),
			)
			.map(([key, value]) => {
				const readable = key
					.replace(/_mean$/, "")
		...
		// Ranking points
		if (
			yearData.breakdown.rp_1_mean !== undefined ||
			yearData.breakdown.rp_2_mean !== undefined
		) {
			lines.push("🏆 RANKING POINTS");
			if (yearData.breakdown.rp_1_mean !== undefined)
				lines.push(
					`RP 1: ${(yearData.breakdown.rp_1_mean * 100).toFixed(1)}% achievement rate`,
				);
			if (yearData.breakdown.rp_2_mean !== undefined)
				lines.push(
					`RP 2: ${(yearData.breakdown.rp_2_mean * 100).toFixed(1)}% achievement rate`,
				);
			lines.push("");
		}
	}
	...
	if (yearData.metrics) {
		...
		if (yearData.metrics.win_prob?.season) {
			...
			lines.push(
				`Win Prediction: ${(wp.acc * 100).toFixed(1)}% accuracy (${wp.count.toLocaleString()} matches)`,
			);
		}
		if (yearData.metrics.score_pred?.season) {
			const sp = yearData.metrics.score_pred.season;
			lines.push(
				`Score Prediction: ${sp.rmse.toFixed(1)} RMSE, ${sp.error.toFixed(1)} avg error`,
			);
		}
		lines.push("");
	}
...
export function formatYearsList(years: YearData[]): string {
	...
	years.forEach((year) => {
		...
		if (year.breakdown) {
			const phases = [];
			if (year.breakdown.auto_points_mean)
				phases.push(`Auto: ${year.breakdown.auto_points_mean.toFixed(1)}`);
			if (year.breakdown.teleop_points_mean)
				phases.push(`Teleop: ${year.breakdown.teleop_points_mean.toFixed(1)}`);
			if (year.breakdown.endgame_points_mean)
				phases.push(`Endgame: ${year.breakdown.endgame_points_mean.toFixed(1)}`);
			...
			if (year.breakdown.rp_1_mean !== undefined || year.breakdown.rp_2_mean !== undefined) {
				const rps = [];
				if (year.breakdown.rp_1_mean !== undefined)
					rps.push(`RP1: ${(year.breakdown.rp_1_mean * 100).toFixed(0)}%`);
				if (year.breakdown.rp_2_mean !== undefined)
					rps.push(`RP2: ${(year.breakdown.rp_2_mean * 100).toFixed(0)}%`);
				lines.push(`   🏆 ${rps.join(" | ")}`);
			}
		}

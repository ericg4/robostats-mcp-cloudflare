...
export function formatTeamEvent(teamEvent: TeamEvent): string {
  ...
  lines.push(`=== Team ${teamEvent.team} "${teamEvent.team_name}" at ${teamEvent.event_name} ===`);
  lines.push("");
  ...
  lines.push(`Confidence Interval: [${teamEvent.epa.conf[0].toFixed(2)}, ${teamEvent.epa.conf[1].toFixed(2)}]`);
  lines.push(`Expected Score: ${teamEvent.epa.total_points.mean.toFixed(1)} ± ${teamEvent.epa.total_points.sd.toFixed(1)}`);
  lines.push("");
  ...
  lines.push(`Record: ${qualRecord.wins}-${qualRecord.losses}-${qualRecord.ties} (${(qualRecord.winrate * 100).toFixed(1)}%)`);
  ...
  if (qualRecord.rps !== null && qualRecord.rps_per_match !== null) {
    lines.push(`Ranking Points: ${qualRecord.rps} total (${qualRecord.rps_per_match.toFixed(1)} per match)`);
  }
  ...
  if (teamEvent.record.elim.count > 0) {
    ...
    lines.push(`Record: ${elimRecord.wins}-${elimRecord.losses}-${elimRecord.ties} (${(elimRecord.winrate * 100).toFixed(1)}%)`);
  ...
  lines.push(`Total Record: ${totalRecord.wins}-${totalRecord.losses}-${totalRecord.ties} (${(totalRecord.winrate * 100).toFixed(1)}%)`);
...
export function formatTeamEventsList(teamEvents: TeamEvent[]): string {
  ...
  teamEvents.forEach((teamEvent, index) => {
    ...
    lines.push(`${index + 1}. Team ${teamEvent.team} "${teamEvent.team_name}" at ${teamEvent.event_name}`);
    lines.push(`   📅 ${teamEvent.year} | Week ${teamEvent.week} | ${teamEvent.type} | ${teamEvent.event}`);
    lines.push(`   📍 ${teamEvent.state ? `${teamEvent.state}, ` : ""}${teamEvent.country} | District: ${teamEvent.district || "N/A"}`);
    
    // Performance summary
    const qualRecord = teamEvent.record.qual;
    const totalRecord = teamEvent.record.total;
    const rankInfo = qualRecord.rank !== null ? `Rank: ${qualRecord.rank}/${qualRecord.num_teams}` : "";
    lines.push(`   🏆 Qual: ${qualRecord.wins}-${qualRecord.losses}-${qualRecord.ties} (${(qualRecord.winrate * 100).toFixed(1)}%) | ${rankInfo}`);
    
    if (teamEvent.record.elim.count > 0) {
      const elimRecord = teamEvent.record.elim;
      lines.push(`   🥇 Elim: ${elimRecord.wins}-${elimRecord.losses}-${elimRecord.ties} (${(elimRecord.winrate * 100).toFixed(1)}%)`);
    }
    
    lines.push(`   📊 EPA: ${teamEvent.epa.norm.toFixed(1)} | Overall: ${totalRecord.wins}-${totalRecord.losses}-${totalRecord.ties} (${(totalRecord.winrate * 100).toFixed(1)}%)`);

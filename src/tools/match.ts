...
export function formatMatch(match: Match): string {
	...
	lines.push(
		`Type: ${match.elim ? "Elimination" : "Qualification"} | Level: ${match.comp_level}`,
	);
	...
	lines.push(
		`Win Probability: Red ${(match.pred.red_win_prob * 100).toFixed(1)}% | Blue ${((1 - match.pred.red_win_prob) * 100).toFixed(1)}%`,
	);
	lines.push(
		`Predicted Score: Red ${match.pred.red_score.toFixed(1)} | Blue ${match.pred.blue_score.toFixed(1)}`,
	);
	lines.push(`Predicted RPs:`);
	lines.push(
		`  Red: RP1 ${match.pred.red_rp_1.toFixed(2)} | RP2 ${match.pred.red_rp_2.toFixed(2)}`,
	);
	lines.push(
		`  Blue: RP1 ${match.pred.blue_rp_1.toFixed(2)} | RP2 ${match.pred.blue_rp_2.toFixed(2)}`,
	);
	lines.push("");
	...
	if (match.result.winner !== "") {
		...
		lines.push(
			`Auto Points: Red ${match.result.red_auto_points} | Blue ${match.result.blue_auto_points}`,
		);
		lines.push(
			`Teleop Points: Red ${match.result.red_teleop_points} | Blue ${match.result.blue_teleop_points}`,
		);
		lines.push(
			`Endgame Points: Red ${match.result.red_endgame_points} | Blue ${match.result.blue_endgame_points}`,
		);
		lines.push(
			`No Fouls: Red ${match.result.red_no_foul ? "✅" : "❌"} | Blue ${match.result.blue_no_foul ? "✅" : "❌"}`,
		);
		lines.push("");

		// Additional result details (game-specific breakdown)
		const otherKeys = Object.keys(match.result).filter(
			(key) =>
				![
					"winner",
					"red_score",
...
export function formatMatchesList(matches: Match[]): string {
	...
	matches.forEach((match, index) => {
		...
		lines.push(
			`   🏁 Type: ${match.elim ? "Elimination" : "Qualification"} | Status: ${match.status}`,
		);
		...
		lines.push(
			`   🎯 Predicted: ${match.pred.winner} (${(match.pred.red_win_prob * 100).toFixed(1)}% Red) | Score: ${match.pred.red_score.toFixed(0)}-${match.pred.blue_score.toFixed(0)}`,
		);

		if (match.result.winner !== "") {
			lines.push(
				`   🏆 Result: ${match.result.winner} wins ${match.result.red_score}-${match.result.blue_score}`,
			);
		} else {

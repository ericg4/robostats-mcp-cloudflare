...
export function formatEvent(event: Event): string {
  ...
  if (event.metrics.win_prob.count > 0 && event.metrics.win_prob.acc !== null) {
    lines.push(`Win Probability: ${event.metrics.win_prob.acc.toFixed(3)} accuracy (${event.metrics.win_prob.count} predictions)`);
  } else {
    lines.push(`Win Probability: N/A`);
  }
  
  if (event.metrics.score_pred.count > 0 && event.metrics.score_pred.rmse !== null) {
    lines.push(`Score Prediction: ${event.metrics.score_pred.rmse.toFixed(2)} RMSE (${event.metrics.score_pred.count} predictions)`);
  } else {
...
export function formatEventsList(events: Event[]): string {
  ...
  events.forEach((event, index) => {
    ...
    lines.push(`   📅 ${event.start_date} to ${event.end_date} | Week ${event.week} | ${event.type}`);
    lines.push(`   📍 ${event.state ? `${event.state}, ` : ""}${event.country} | District: ${event.district || "N/A"}`);
